...
pipeline {
    ...
    environment {
        ...
        REGEXP_BRANCH_USER_DEFINED_INPUT = ".*"
        REGEXP_BRANCH_FEATURE = "feature/.*"
        REGEXP_BRANCH_RELEASE = "release/.*"
        REGEXP_BRANCH_HOTFIX = "hotfix/.*"
        REGEXP_BRANCH_BUGFIX = "bugfix/.*"
        ...
        APP_ARTIFACT_DIR_DEV = "app_artifact_dev"
        APP_ARTIFACT_DIR_QA = "app_artifact_qa"
        APP_ARTIFACT_DIR_STAGING = "app_artifact_staging"
        APP_ARTIFACT_DIR_PROD = "app_artifact_prod"

        // Tags symantic versioning
        REGEXP_TAG_BETA = "refs/tags/v\\d+\\.\\d+\\.\\d+-beta\\.\\d+"
        REGEXP_TAG_RC = "refs/tags/v\\d+\\.\\d+\\.\\d+-rc\\.\\d+"
        REGEXP_TAG_STABLE = "refs/tags/v\\d+\\.\\d+\\.\\d+"
        ...
        // Jenkins user defined tags
        TAG_USER_DEFINED_REGEXP_BETA = "v\\d+\\.\\d+\\.\\d+-beta\\.\\d+"
        TAG_USER_DEFINED_REGEXP_RC = "v\\d+\\.\\d+\\.\\d+-rc\\.\\d+"
        TAG_USER_DEFINED_REGEXP_STABLE = "v\\d+\\.\\d+\\.\\d+"
        ...
        JOB_NAME_DEPLOY_DEV = "Deploy-Dev"
            ...
            steps {
                ...
                dir("${APP_SOURCE}") {
                    ...
                    script {
                        ...
                        // Case: Dev
                        } else if ( jobName == env.JOB_NAME_DEPLOY_DEV || jobName == JOB_NAME_DEPLOY_PLAYGROUND_QA ) {
                        ...
                        // Create patch release
                            ...
                            if ( numberOfReleaseBranches != "0" ) {
                                ...
                                // Set target commit with latest stable tag
        ...
        stage("Build") {
            ...
            when {
                ...
                anyOf {
                    ...
                    expression { jobName == env.JOB_NAME_DEPLOY_DEV }
        ...
        stage("Create Artifact") {
            ...
            parallel {
                ...
                stage ("Dev") {
                    ...
                    steps {
                        dir ("${APP_ARTIFACT_DIR_DEV}") {
                            // Clone artifact repo
                            git credentialsId: env.ARTIFACT_GIT_CRED_ID, url: env.ARTIFACT_GIT_URL
                            // Create artifact branch or checkout current one
                            sh "git checkout ${targetCommit} || git checkout -b ${targetCommit}"
                        }

                        // Sync from source to artifact
                        sh "rsync -rtvq --delete --exclude .git/ --exclude .gitignore --exclude .gitignore.docroot --exclude .gitlab-ci.yml --links ./${APP_SOURCE}/ ./${APP_ARTIFACT_DIR_DEV}/"
                        ...
                        dir ("${APP_ARTIFACT_DIR_DEV}") {
                            ...
                            sh "git status && \
                                git add . && \
                                git commit -qm \"Artifact built from ${targetCommit}\" || echo \"Nothing to commit, working tree clean\" && \
                                git push -u origin ${targetCommit}"
                        }
                    }
                }
                ...
                stage ("QA") {
                    ...
                    steps {
                        dir ("${APP_ARTIFACT_DIR_QA}") {
                            ...
                            sh "rsync -rtvq --delete --exclude .git/ --exclude .gitignore --exclude .gitignore.docroot --exclude .gitlab-ci.yml --links ./${APP_SOURCE}/ ./${APP_ARTIFACT_DIR_QA}/"
